...
struct NinjaMain : public BuildLogUser {
  NinjaMain(const char* ninja_command, const BuildConfig& config) :
      ...
      dupe_edge_action_(kDupeEdgeActionError) {}
...
bool NinjaMain::RebuildManifest(const char* input_file, string* err,
  ...
  Builder builder(&state_, config_, &build_log_, &deps_log_, &disk_interface_,
                  status, start_time_millis_, dupe_edge_action_);
...
"  dupbuild={err,warn}    multiple rules generate the same output\n"
"  phonycycle={err,warn}  phony build statement references itself\n"
    ...
    const char* suggestion = SpellcheckString(name.c_str(), "dupbuild=err",
                                              "dupbuild=warn", "phonycycle=err",
...
NORETURN void real_main(int argc, char** argv) {
  ...
  for (int cycle = 1; cycle <= kCycleLimit; ++cycle) {
    ...
    ninja.dupe_edge_action_ = parser_opts.dupe_edge_action_;
    ...
    if (ninja.RebuildManifest(options.input_file, &err, status)) {
      ...
      // If the error is about multiple rules generate, just warn instead of error
      // when dupbuild=warn is specified
      if (!options.dupe_edges_should_err && err.find("multiple rules generate") != std::string::npos) {
        status->Warning("%s", err.c_str());
        // Continue with the build despite the duplicate rules
      } else {
